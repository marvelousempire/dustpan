...
export function ProcessLeaderboard({ rows }: { rows: PerformanceProcess[] }) {
  return (
    <div className="max-h-[340px] overflow-auto">
      <div className="grid grid-cols-[52px_minmax(0,1fr)_120px_112px] gap-2 pb-2 text-[10px] font-semibold uppercase tracking-[0.08em] text-fg-faint">
        <span>PID</span><span>Process</span><span>CPU</span><span>RAM</span>
      </div>
      {rows.slice(0, 18).map((proc) => (
        <div key={`${proc.pid}-${proc.name}`} className="grid grid-cols-[52px_minmax(0,1fr)_120px_112px] gap-2 border-t border-border/10 py-1.5 text-[12px]">
